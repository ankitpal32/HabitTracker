...
function Progress() {
  ...
  const getHabitCompletionPercentage = (habit) => {
    ...
    const completedCount = completedDays.filter((date) => completedDates.includes(date)).length;
    const isWeekly = habit.frequency && habit.frequency.toLowerCase() === "weekly";
    const expectedCompletions = isWeekly ? Math.max(1, Math.round(range / 7)) : range;

    return Math.min(100, Math.round((completedCount / expectedCompletions) * 100));
  };
    ...
    return (
      <div className="progress-page">
        ...
        <div className="empty-card">
          ...
          <p>Getting your latest habit activity.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="progress-page">
      <div className="page-heading">
        <div>
          <p className="page-label">INSIGHTS</p>
          <h1>Your Progress</h1>
          <p>See how consistently you're building your habits.</p>
        </div>
      </div>
      ...
      <section className="overview">
        <div className="stat-card" style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            ...
            <strong className="stat-number" style={{ margin: 0 }}>{todayProgress}%</strong>
            <span className="stat-description">Daily completion</span>
          </div>
      ...
      <section className="progress-card">
        <div className="progress-card-heading">
          <div>
            <h2>Activity</h2>
            <p>Your habit activity over time.</p>
          </div>

          <div className="range-buttons">
            <button
              type="button"
              className={range === 7 ? "range-button active" : "range-button"}
              onClick={() => setRange(7)}
            >
              7 Days
            </button>
            <button
              type="button"
              className={range === 30 ? "range-button active" : "range-button"}
              onClick={() => setRange(30)}
            >
              30 Days
            </button>
        ...
        {totalHabits === 0 ? (
          <div className="empty-card">
            ...
            <p>Create a habit and start completing it to see your activity here.</p>
          </div>
        ) : (
          <div className={range === 30 ? "activity-chart month-chart" : "activity-chart"}>
            {activityData.map((day) => {
              const percentage = totalHabits === 0 ? 0 : Math.round((day.completed / totalHabits) * 100);
              return (
                <div
                  className={`activity-day ${day.isToday ? "today" : ""}`}
                  key={day.date}
                  title={`${day.completed} habit${day.completed === 1 ? "" : "s"} completed on ${day.date}`}
                >
                  <div className="activity-bar">
                    <div
                      className={day.completed > 0 ? "filled" : "empty"}
                      style={{ height: `${percentage === 0 ? 6 : percentage}%` }}
                    ></div>
                  </div>
                  <span>{day.label}</span>
      ...
      <section className="progress-card">
        <div className="progress-card-heading">
          <div>
            ...
            <p>See how each habit is doing.</p>
          </div>
        </div>
        ...
        {habits.length === 0 ? (
          <div className="empty-card">
            <div className="empty-icon"><FiBarChart2 /></div>
            <h3>No habits yet</h3>
            <p>Your habit performance will appear here once you create your first habit.</p>
          </div>
        ) : (
          <div className="performance-list">
            {habits.map((habit) => {
              const percentage = getHabitCompletionPercentage(habit);
              return (
                <div className="performance-item" key={habit._id}>
                  <div className="performance-top">
                    <div>
                      <strong>{habit.name}</strong>
                      <span>{habit.frequency}</span>
                    </div>
                    <div className="performance-streak">🔥 {habit.streak || 0}</div>
                  </div>
